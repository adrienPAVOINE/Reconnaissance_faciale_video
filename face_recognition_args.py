...
pose_predictor_68_point = dlib.shape_predictor("C:/Users/axbarou/Desktop/pretrained_model/shape_predictor_68_face_landmarks.dat")
pose_predictor_5_point = dlib.shape_predictor("C:/Users/axbarou/Desktop/pretrained_model/shape_predictor_5_face_landmarks.dat")
# Maps human faces into vectors where pictures of the same person are mapped near to each
face_encoder = dlib.face_recognition_model_v1("C:/Users/axbarou/Desktop/pretrained_model/dlib_face_recognition_resnet_model_v1.dat")
# Face detector
...
parser.add_argument('-v', '--video', type=str, required=True, help='directory of input videos')
...
print('[INFO] Importing videos...')
videos_path = Path(args.video)
videos = [file_ for file_ in videos_path.rglob('*.mp4')]
vid_path = []
for vid in videos: 
    vid_path = vid_path + [str(vid).replace( '\\', '/')] #re.sub("\\", "/", str(vid)) #'/'.join(vid.split('\\'))#.replace( os.sep, '/')
...
for i in range(len(vid_path)):
    print('[INFO] video no°{}'.format(i+1))
    video = cv2.VideoCapture(vid_path[i])
    ...
    out = cv2.VideoWriter('output{}.avi'.format(i), fourcc, fps, (w,h))
